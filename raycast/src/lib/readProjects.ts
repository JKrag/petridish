...
export const DEFAULT_STATE_PATH = path.join(os.homedir(), ".petridish", "projects.json");
...
export function readRadar(statePath: string = DEFAULT_STATE_PATH): Radar {
  if (!fs.existsSync(statePath)) {
    throw new StateFileMissingError(`no state file at ${statePath}; run 'swab scan' first`);
  }
