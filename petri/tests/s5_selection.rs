...
fn new_selects_first_visible_row() {
    ...
    let state = BrowserState::new(&radar);
    assert!(!state.visible.is_empty(), "minimal.json must have a visible project");
    assert_eq!(state.selected, Some(0), "initial selection must be the first visible row");
}
...
fn empty_visible_list_has_no_selection_and_does_not_panic() {
    ...
    assert_eq!(state.selected, None, "empty visible list must select None, not panic or default to Some(0)");
...
fn move_selection_clamps_at_the_bottom_never_wraps() {
    ...
    assert_eq!(state.selected, Some(last), "must clamp at the last visible row, never wrap");
}
...
fn move_selection_clamps_at_the_top_never_wraps() {
    ...
    assert_eq!(state.selected, Some(0), "must clamp at the first visible row, never wrap");
}
...
fn move_selection_crosses_section_boundaries() {
    ...
    let mut state = BrowserState::new(&radar);
    use petridish_core::schema::StatusBucket;
    let bucket_at = |state: &BrowserState, pos: usize| radar.projects[state.visible[pos]].status_bucket;

    let start_bucket = bucket_at(&state, 0);
    assert_eq!(start_bucket, StatusBucket::Active, "loaded.json's first visible row must be active");
    ...
    assert!(crossed, "move_selection must cross from the active section into the next one");
}
...
fn apply_filter_case_insensitive_substring_match() {
    ...
    let needle = target_name.chars().take(3).collect::<String>().to_uppercase();
    ...
    for &idx in &state.visible {
        assert!(
            radar.projects[idx].name.to_lowercase().contains(&needle.to_lowercase()),
...
fn apply_filter_empty_query_returns_full_unfiltered_list() {
    ...
    assert!(state.visible.len() < full.len(), "sanity: the nonsense query must actually filter something out");
    state.apply_filter(&radar, "");
    assert_eq!(state.visible, full, "an empty query must return the input (full grouped list) unchanged");
}
...
fn apply_filter_out_the_selected_project_resets_to_first_available_row() {
    ...
    let selected_name = radar.projects[state.visible[state.selected.unwrap()]].name.clone();
    ...
    assert_eq!(state.selected, None, "filtering out everything (including the selection) must not panic and must clear selection");
}
...
fn apply_filter_keeps_selection_on_the_same_project_when_it_survives() {
    ...
    state.apply_filter(&radar, &needle);
    let new_pos = state.selected.expect("selected project matches its own name substring, must stay selected");
    assert_eq!(
