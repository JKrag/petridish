...
the Cinnamon panel applet (`integrations/cinnamon/`) and the SNI tray indicator
(`integrations/sni-tray/`, any StatusNotifierItem host: COSMIC, KDE, Waybar) — both
manually installed. The xbar *plugin file* install step
...
- **`petridish-cli/`** (Rust, published as `petri-dish`, binary **`petridish`**) is the
  ...
  no OS gate (issue #25's macOS-only refusal predated any Linux consumer) and always exits
   0. Its Linux consumers are the Cinnamon panel applet in `integrations/cinnamon/` and the
  SNI tray indicator in `integrations/sni-tray/` — manually installed, not general menu-bar
...
- `integrations/` holds the non-Rust frontends: `xbar/` (docs; the plugin itself is
  generated by `petridish install`), `raycast/` (a TypeScript extension, gated in CI) and
  `cinnamon/` (a Cinnamon panel applet over `petridish menubar`; its parser is gated in CI
  via `make cinnamon`) and `sni-tray/` (a Python StatusNotifierItem indicator over the same
  command, for COSMIC/KDE/Waybar; parser gated via `make sni-tray`). The two parsers read
  the same menubar.rs output and their test suites mirror each other — keep them agreeing.
